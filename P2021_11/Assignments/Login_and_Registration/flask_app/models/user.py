...
class User:
    ...
    def get_by_email(cls,data):
        ...
        user_db = connectToMySQL("log_reg").query_db(query,data)
    ...
    def validate_user(user):
        is_valid = True
        query = "SELECT * FROM users WHERE email = %(email)s;"
        results = connectToMySQL("log_reg").query_db(query,user)
        if len(results) >= 1:
        ...
        if len(user["first_name"]) < 1:
            flash("We need your first name!")
            is_valid = False
        if len(user["last_name"]) < 1:
            flash("We need your last name too!!!")
            is_valid = False
